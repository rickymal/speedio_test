...
def csv_to_dict_converter(chunk: list[str],):
    ...
    try:
        ...
        for ind, raw_document in enumerate(chunk):
            ...
            """
            O Layout presente no site afirma que a informação é separada 
...
def list_cnae_converter(chunk: list[dict]):
    try:
        ...
        for data in chunk:
            ...
            cnae_sec_splitted = [x.strip() for x in cnae_sec.split(
                ',') if x.strip().isdigit()]
...
@pipeline.pipe_function
def convert_cnae_corr_document_to_dataframe(data: list[dict]):
...
def create_corr_table(data: dataframe):
    ...
    # a primeira máscara filtra códigos que aparecem tanto no atributo 'cnae fiscal principal' quanto 'todos os cnaes'
    ...
    Dessa forma pode-se observar na diagonal da primeira parte
    ...
    percentage_of_pair = all_data.apply(
        lambda x: x / complete_data).round(decimals=3)
